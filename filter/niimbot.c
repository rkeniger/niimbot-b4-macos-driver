/*
 * niimbot.c - NIIMBOT B4 packet protocol core. See niimbot.h.
 */
...
/* ---- state ---------------------------------------------------------- */
volatile sig_atomic_t niimbot_cancelled = 0;

static niimbot_io_t io;
static niimbot_job_t job;
...
static void on_term(int sig) { (void)sig; niimbot_cancelled = 1; }
...
{
  if (s <= 0 || job.dry_run) return;
  if (s > 600) s = 600;
  struct timespec ts = { (time_t)s, (long)((s - (double)(time_t)s) * 1e9) };
  while (nanosleep(&ts, &ts) != 0 && errno == EINTR && !niimbot_cancelled) { }
}

int niimbot_last_error(void) { return printer_error; }

const char *niimbot_error_name(int code)
{
  return (code > 0 && code < N_ERROR_NAMES && error_names[code]) ? error_names[code] : "unknown";
}
...
static void send_packet(unsigned char cmd, const unsigned char *data, size_t len)
{
  ...
  if (io.write(io.ctx, hdr, 4) != 0 || (len && io.write(io.ctx, data, len) != 0)
      || io.write(io.ctx, tail, 3) != 0) {
...
static void note_packet(unsigned char cmd, const unsigned char *data, size_t len)
{
  if (cmd == RSP_ERROR && len >= 1) {
    ...
    page_index = (unsigned)((data[0] << 8) | data[1]);
  }
}
...
static int read_packet(unsigned char *cmd, unsigned char *data, size_t *len, double timeout)
{
  for (;;) {
    ...
    ssize_t got = io.read(io.ctx, rxbuf + rxlen, sizeof(rxbuf) - rxlen, timeout);
    if (got <= 0) return 0;
    rxlen += (size_t)got;
...
void niimbot_fail(const char *msg)
{
  if (printer_error) {
    const char *name = niimbot_error_name(printer_error);
    if (printer_error == 1) fputs("STATE: +cover-open-error\n", stderr);
    if (printer_error == 2 || printer_error == 8)
      fprintf(stderr, "STATE: +%s\n", job.media_out_reason ? job.media_out_reason : "media-empty-error");
                    ...
                    unsigned char resp, const char *what, double timeout)
{
  ...
  if (niimbot_transceive(cmd, req, reqlen, resp, data, &len, timeout)) {
    if (len >= 1 && data[0] == 0 && cmd != CMD_PRINT_STATUS)
      niimbot_fail(what);
    return;
  }
  if (printer_error) niimbot_fail(what);
  if (job.ack && ack_available == 1) {
...
static void check_line(unsigned line)
{
  ...
  niimbot_transceive(CMD_CHECK_LINE, d, 3, RSP_CHECK_LINE, data, &len, 1.0);
  if (printer_error) niimbot_fail("printer reported an error while receiving image data");
}
...
static void wait_print_complete(unsigned copies, double timeout)
{
  ...
  while (now_s() < deadline && !niimbot_cancelled) {
    drain();
    if (printer_error) niimbot_fail("printer reported an error while printing");
    if (page_index >= copies) return;
    if (!niimbot_transceive(CMD_PRINT_STATUS, (const unsigned char *)"\x01", 1, RSP_PRINT_STATUS, st, &len, 3.0)) {
      if (printer_error) niimbot_fail("printer reported an error while printing");
      continue;
    }
    if (len >= 4) {
      unsigned page = (unsigned)((st[0] << 8) | st[1]);
...
/* ---- public API ----------------------------------------------------- */
void niimbot_init(const niimbot_io_t *the_io, const niimbot_job_t *the_job)
{
  io = *the_io;
  job = *the_job;
  if (job.density < 1) job.density = 1;
  if (job.density > 5) job.density = 5;
  if (job.label_type != 1 && job.label_type != 2 && job.label_type != 5) job.label_type = 1;
  if (job.dry_run) job.ack = 0;
...
void niimbot_connect(void)
{
  ...
  if (niimbot_transceive(CMD_CONNECT, (const unsigned char *)"\x01", 1, RSP_CONNECT, d, &n, 2.0))
...
void niimbot_print_page(cups_raster_t *ras, const cups_page_header2_t *hdr, unsigned page, unsigned copies)
{
  ...
  fprintf(stderr, "DEBUG: page %u: %ux%u px, %u bpp, cs %d, bpl %zu, copies %u, density %d, label type %d\n",
          page, cols, rows, hdr->cupsBitsPerPixel, hdr->cupsColorSpace, bpl, copies, job.density, job.label_type);

  /*
   * The raster header is untrusted: any local user can hand cupsd a
   * hand-made application/vnd.cups-raster file (or any IPP client a PWG
   * raster file), and libcups does not cross-check these fields. Reject
   * anything inconsistent before it is used to size buffers or protocol fields.
   */
  if (hdr->cupsBitsPerPixel != 1 && hdr->cupsBitsPerPixel != 8)
    niimbot_fail("Unsupported raster format (need 1-bit or 8-bit grayscale)");
  if (hdr->cupsBitsPerPixel == 1 && hdr->cupsBitsPerColor != 1)
    niimbot_fail("Unsupported raster format (1-bit pixel must be 1-bit color)");
  if (hdr->cupsBitsPerPixel == 8 && hdr->cupsBitsPerColor != 8)
    niimbot_fail("Unsupported raster format (8-bit pixel must be 8-bit color)");
  if (hdr->cupsColorSpace != CUPS_CSPACE_K && hdr->cupsColorSpace != CUPS_CSPACE_W
      && hdr->cupsColorSpace != CUPS_CSPACE_SW)
    niimbot_fail("Unsupported raster color space (need K or W grayscale)");
  if (cols == 0 || rows == 0) niimbot_fail("Empty page");
  if (rowbytes > 249) niimbot_fail("Page wider than the printer supports");   /* 6-byte row header + data <= 255 */
  if (rows > 65535) niimbot_fail("Page longer than the printer supports");    /* protocol u16 */
  if (bpl == 0 || bpl > (64u * 1024u)) niimbot_fail("Invalid raster bytes-per-line");
  if (hdr->cupsBitsPerPixel == 1 && bpl < rowbytes) niimbot_fail("Invalid raster bytes-per-line for width");
  if (hdr->cupsBitsPerPixel == 8 && bpl < cols) niimbot_fail("Invalid raster bytes-per-line for width");
  if (copies == 0 || copies > 999) niimbot_fail("Too many copies (max 999)");

  unsigned char *line = malloc(bpl);
  unsigned char *row = malloc(rowbytes);
  unsigned char *prev = malloc(rowbytes);
  if (!line || !row || !prev) niimbot_fail("Out of memory");

  /* job/page setup */
  unsigned char d[16];
  d[0] = (unsigned char)job.density;
  control(CMD_SET_DENSITY, d, 1, RSP_SET_DENSITY, "Unable to set density", 2.0);
  d[0] = (unsigned char)job.label_type;
  ...
  int have_prev = 0;
  for (y = 0; y < rows; y++) {
    if (cupsRasterReadPixels(ras, line, (unsigned)bpl) < 1) niimbot_fail("Short raster data");
    if (hdr->cupsBitsPerPixel == 1) {
      /* CUPS K colorspace: 1 = black, MSB first: exactly what the printer wants */
      memcpy(row, line, rowbytes);
      if (hdr->cupsColorSpace != CUPS_CSPACE_K) for (size_t i = 0; i < rowbytes; i++) row[i] = (unsigned char)~row[i];
    } else {
